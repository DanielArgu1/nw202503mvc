...
class Security extends \Dao\Table
{
    ...
    static public function newUsuario($email, $password)
    {
        ...
        $sqlIns = "INSERT INTO `usuario` (`useremail`, `username`, `userpswd`,
            ...
            now(), :usertipo);";
            $rowsAffected = self::executeNonQuery($sqlIns, $newUser);

            if ($rowsAffected > 0) {
                $usuario = self::getUsuarioByEmail($email);
                $newUserId = $usuario["usercod"];
                $sqlRoleIns = "INSERT INTO `roles_usuarios` 
                       (`usercod`, `rolescod`, `roleuserest`, `roleuserfch`, `roleuserexp`)
                       VALUES (:usercod, :rolescod, 'ACT', NOW(), DATE_ADD(NOW(), INTERVAL 1 YEAR));";
                    $rolAdminParams = array(
                         "usercod" => $newUserId,
                         "rolescod" => "Publico" 
                        );
                self::executeNonQuery($sqlRoleIns, $rolAdminParams);
            }
